...
Esta etapa (Prompt 1) entrega a **estrutura do projeto**: árvore de diretórios,
`style.css` base (reset + variáveis globais) e o esqueleto HTML/JS com a
navegação simulada entre `index.html` (login) e `dashboard.html`.
O design avançado chega nos prompts seguintes.
...
Use qualquer e-mail e senha: a autenticação é simulada e a "sessão" fica no
LocalStorage.
...
| --- | --- |
| `index.html` | Ponto de entrada. Marcação semântica do formulário de login (e-mail, senha, "Lembrar-me", "Criar conta"). Carrega `style.css`, `components.css`, `login.css` e a cadeia de scripts terminando em `login.js`. |
...
| `css/components.css` | Componentes compartilhados entre as telas: botões, inputs, modais, toasts, skeletons. Consome exclusivamente as variáveis de `style.css`. |
| `css/login.css` | Estilos específicos da tela de login: card com glassmorphism, brilho neon, animações de entrada, responsividade. |
...
| `js/login.js` | Liga o formulário de login ao fluxo: checa campos preenchidos, cria a sessão e redireciona para o dashboard. Validação rica e animações vêm no Prompt 2. |
...
- **Prompt 2** — tela de login futurista (glassmorphism, neon, animações).
